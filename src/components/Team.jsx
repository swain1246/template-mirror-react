...
const Team = () => {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8 px-4 sm:px-6 md:px-8 lg:px-16 py-8 sm:py-10">
      {teamMembers.map((member) => (
        <div key={member.id} className="bg-white shadow-md overflow-hidden relative group">
          ...
          {/* Member Info */}
          <div className="bg-gray-900 p-4 sm:p-5 text-center">
            <h3 className="font-bold text-lg sm:text-xl text-gray-100">{member.name}</h3>
            <p className="text-gray-400 text-sm sm:text-base">{member.designation}</p>
          </div>
          
          {/* Hover Overlay */}
          <div className="absolute inset-0 bg-gray-800 bg-opacity-90 flex flex-col items-center justify-center p-4 sm:p-6 transform translate-y-full group-hover:translate-y-0 transition-transform duration-500 ease-in-out">
            <h3 className="font-bold text-lg sm:text-xl text-white mb-2">{member.name}</h3>
            <p className="text-gray-300 mb-3 text-sm sm:text-base">{member.designation}</p>
            <p className="text-gray-400 text-xs sm:text-sm text-center mb-6 line-clamp-4">{member.description}</p>
            
            {/* Social Icons */}
            <div className="flex space-x-3 sm:space-x-4">
              <a href="#" className="w-8 h-8 sm:w-10 sm:h-10 bg-gray-700 rounded-full flex items-center justify-center text-white hover:bg-yellow-500 transition-colors duration-300">
